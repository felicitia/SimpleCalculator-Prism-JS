...
NSString * SimpleCalculator$ModuleRequireFormat = @"(function(exports){"
...
@implementation SimpleCalculatorObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_simplecalculator);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create SimpleCalculator global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "SimpleCalculator" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_simplecalculator = [[SimpleCalculatorObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_simplecalculator];
	
	NSString *_simplecalculatorNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _simplecalculatorNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_simplecalculator valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:SimpleCalculator$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.simplecalculator.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.simplecalculator.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
