...
//Rare exceptions to indicate a bug in the _simplecalculator code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_simplecalculator = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_simplecalculator]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.simplecalculator.module"
